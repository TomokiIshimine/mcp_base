...
| infrastructure | `src/infrastructure/config.py` | `MySQLConfig`。接続設定を環境変数から構築。必須値欠如は `ConfigError` で起動失敗（fail-fast）。 |
...
- `MySQLConfig.from_env()` から `MySQLGreetingCrudRepository` を構築し、`ManageGreetingsUseCase` → `GreetingCrudController` の順に注入して `render_crud` を呼ぶ。
- リポジトリ（接続プール）は `@st.cache_resource` でプロセス内 1 回だけ構築し再利用する。
- 設定不備（`ConfigError`）は捕捉せず起動を失敗させる（fail-fast）。描画中の想定外例外は最上位で 1 回だけ捕捉し、ログ記録と汎用メッセージ表示に落とす。
