...
2. 入力を Read する: 実装差分 / `<workdir>/04_design.md` / `<workdir>/03_requirements.md` / 担当 `<aspect>` / `<iteration>`（出力レポートの決定論的命名に用いる反復番号）。2 周目以降は `previous-review-path`（前周回の同観点レポート絶対パス）も渡されるため、`/multi-aspect-review` の差分レビューモード契約に従って前回指摘の解消状況を点検する。`architecture-fidelity` 担当時は上記「埋め込み済みプロジェクト固有情報」も判断材料に含める。
