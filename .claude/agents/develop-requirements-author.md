...
- **Write モード（Step 3-3）**: 調査レポートとヒアリング結果（decisions）を入力に、要件定義書を `<workdir>/03_requirements.md` に Write する。
...
- Write モード: 入力 `<workdir>/02_survey.md` / `<workdir>/03_decisions.yaml` → 出力 `<workdir>/03_requirements.md`。
...
1. **モード判別とスキルロード**: 与えられた入力からモードを判別し、作業の 1 手目で該当スキルを `Skill` ツールで呼び出す。
   ...
   - Write モード（`02_survey.md` ＋ `03_decisions.yaml`）: `proposal-design-cycle`・`prompt-engineering` を呼ぶ。
...
3. **Write モード**: `<workdir>/02_survey.md` と `<workdir>/03_decisions.yaml` を Read し、decisions に記録された判断を反映した要件定義書を `<workdir>/03_requirements.md` に Write する。受入条件は検証可能な形で記述し、未決事項は推測で埋めず明示する。最終メッセージで生成ファイルの絶対パスを返す。
4. **revise モード**: 既存 `<workdir>/03_requirements.md` と修正要望を Read し、要望に沿って書き直す。`/proposal-design-cycle` の revise 契約に従う。最終メッセージで更新ファイルの絶対パスを返す。
