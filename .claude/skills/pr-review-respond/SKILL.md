...
対象 PR 番号は、`pr-review-analyst` の最終メッセージ（`対象 PR: #<N>`）と上記 `## 評価サマリ` の双方から取得できる。メインはこの値を `<pr-number>` として保持し、Step 1.6 のブランチ整合（明示起動時のチェックアウト、無引数起動時の head 一致再確認）と Step 5 の返信投稿へ引き回す。
...
- 呼び出し: `workflow-init`
- 渡す引数: `workflow-name: pr-review-respond`
...
## Step 1.6 — ブランチ整合プリフライト（指摘収集・評価の前段）

Step 2 の `pr-review-analyst` は各指摘を**チェックアウト中のソースツリー**に照らして対応要否を判定するため、ブランチ整合（対象 PR の head のチェックアウト）は指摘収集・評価より**前**に置き、analyst が正しいリビジョンで評価することを保証する。整合を Step 2 の後段に置くと、明示起動で別ブランチから起動した場合に誤ったソースツリーで評価してしまう（指摘 R7）。起動パスにより順序差があるため以下で扱う。

- **明示起動（`<pr-number>` あり）**: メインが `gh pr view <pr-number> --json headRefName` で対象 PR の head ブランチを解決する。現在ブランチが head と一致すればそのまま続行し、不一致なら head ブランチをチェックアウトしてから Step 2 へ進む。これにより analyst が対象 PR head のソースツリーで評価する。
- **無引数起動（`<pr-number>` なし）**: PR 番号は Step 2 の `pr-review-analyst` が `gh pr view`（現在ブランチ）で確定するため、この時点では未確定。無引数時は現在ブランチが対象 PR の head である前提が成立する（`gh pr view` は現在ブランチに紐づく PR を解決する）ため、チェックアウトせず現在ブランチのまま Step 2 へ進む。Step 2 で PR 番号確定後、メインは `gh pr view <pr-number> --json headRefName` で head と現在ブランチの一致を再確認し、万一不一致なら中断してユーザー指示を仰ぐ。
- チェックアウトが安全に行えない（未コミット変更が残る等）場合は中断し、ユーザーへ状況を提示して指示を仰ぐ（`/workflow-orchestration` の中断ポリシーに準拠）。通常は Step 1.5 のクリーンワークツリーゲート通過後のため安全にチェックアウトできる。
...
- 期待する戻り値: `<workdir>/01_review-assessment.md` の絶対パスと、解決した対象 PR 番号（最終メッセージの `対象 PR: #<N>`）。メインはこの PR 番号を `<pr-number>` として確定・保持し、以後 Step 5 へ引き回す。無引数起動時は、ここで確定した PR 番号で Step 1.6 のブランチ整合（head と現在ブランチの一致再確認）を行う。評価レポートは指摘一覧＋判定＋根拠＋対応分の修正方針を持ち、先頭に承認ゲート用の固定見出し `## 評価サマリ`（`- 対象 PR 番号: <N>` 行を含む）を備える。
...
- **入力写像（本ワークフロー固有の差分）**: 流用 3 体（`develop-coder` / `develop-reviewer` / `develop-runner`）は `develop` 由来の入力パスをハードコードで Read する。すなわち `develop-coder` は `<workdir>/04_design.md`、`develop-reviewer` は `<workdir>/04_design.md` と `<workdir>/03_requirements.md`、`develop-runner` は `<workdir>/04_design.md` を Read する。これら実ファイルは Step 3 承認後に `pr-review-fix-plan-mapper` が写像生成済みである（メインは Write しない）。実体化を省くと流用 3 体は入力欠落でループ前に停止するため、Step 4 起動前に 3 ファイルの存在を前提とする。
  - 流用 3 体への引数供給元を Step 4 内で明示する: `develop-runner` の `<requirement>` には `pr-review-fix-plan-mapper` が写像生成した `<workdir>/03_requirements.md` の絶対パスを渡す（runner はこれをファイル Read するため、メインは限定 Read 契約を破らずに requirement を供給でき、空ないし即席の requirement で検証する経路を塞ぐ）。`develop-reviewer` の `<aspect>` は固定 6 観点を 1 体 1 観点で割り当て、`output-path` / `previous-review-path` は `/multi-aspect-review` 契約の命名規約で具体化する。
...
- **反復制御**: 本ワークフローの `<iteration-limit>` は **5 周**（`/multi-aspect-review` の推奨値であり、流用元 `develop` ワークフローとも整合する）。FAIL の観点・FAIL の動作確認がある限り、当該 FAIL を入力に `develop-coder` を再実装起動し、再点検する。全観点 PASS かつ動作確認 PASS で抜ける。`<iteration-limit>` に達しても全 PASS に至らない場合は、`/multi-aspect-review` の「反復上限到達時の扱い」に従いその時点で打ち切り、最終周回のレビューレポート・動作確認レポートのパス一覧をユーザーに提示して指示を仰ぐ。上限到達は失敗ではなくユーザー引き継ぎであり、メインは自動リトライしない（契約本文は DRY のため再掲せず参照に留める）。上限到達時も全 PASS 未達のため、Step 5 の push・返信投稿（不可逆な副作用）へは進まない。レビューレポート・動作確認レポートのパスとテンプレートは `/multi-aspect-review` 契約を正本とする（本スキルで固定しない）。
...
- **push（メインが実施）**: コミット後に、Step 1.6 で確定した対象 PR の head ブランチ `<work-branch>` を `git push` する。push 対象が対象 PR の head と一致することを前提とし、push は本ステップでメインが実施し、`pr-review-responder` は実施しない前提。
...
- Step 3 が未承認のまま Step 4 以降へ進まない。Step 4 の改善ループが全 PASS に至らないうち（`<iteration-limit>` 到達による打ち切りを含む）は Step 5 の push・返信投稿を行わない（不可逆な副作用を承認・品質確認の後段に閉じる）。反復上限到達時の扱いは Step 4 の反復制御に従う。
