...
- **承認後**: メインが確定対応方針を `<workdir>/02_fix-plan.md` に記録する。これは人間可読の確定対応方針であり、流用 3 体への入力実ファイルは Step 4 冒頭で本ファイルから写像生成する（後述の入力写像）。
...
- **入力写像（本ワークフロー固有の差分・Step 4 冒頭で必ず実施）**: 流用 3 体（`develop-coder` / `develop-reviewer` / `develop-runner`）は `develop` 由来の入力パスをハードコードで Read する。すなわち `develop-coder` は `<workdir>/04_design.md`、`develop-reviewer` は `<workdir>/04_design.md` と `<workdir>/03_requirements.md`、`develop-runner` は `<workdir>/04_design.md` を Read する。したがって本ワークフローは、ループ起動の前にメインが `<workdir>/02_fix-plan.md` の内容を次の実ファイルへ **写像生成（Write）** する。この実体化を省くと流用 3 体は入力欠落でループ前に停止する。
  - `<workdir>/03_requirements.md`: 対応する各指摘を満たすべき要件として整形（`develop-reviewer` の充足判定の根拠）。
  - `<workdir>/04_design.md`: 冒頭に「領域定義」セクション（`develop-coder` の実装範囲の唯一の根拠）と「テスト設計」セクション（`develop-runner` の動作確認の根拠）を備える。
  - 流用 3 体への引数供給元も Step 4 内で明示する: `develop-runner` の `<requirement>` には対応要件サマリを渡し、`develop-reviewer` の `<aspect>` は固定 6 観点を 1 体 1 観点で割り当て、`output-path` / `previous-review-path` は `/multi-aspect-review` 契約の命名規約で具体化する。
  - develop 側の成果物契約（入力パス・セクション名）が変わった場合は本写像も追従が要る（疎結合上の既知の依存）。
...
- **コミット（メインが実施）**: Step 4 改善ループ脱出後・push 前に、実装差分をコミットする。コミット粒度・メッセージはメインが対応方針に基づき決め、必要に応じ `AskUserQuestion` でユーザーに確認する。
- **push（メインが実施）**: コミット後に作業ブランチを `git push` する。push は本ステップでメインが実施し、`pr-review-responder` は実施しない前提。
