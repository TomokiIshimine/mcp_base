...
_MAX_CONNECTIONS = 5
# 接続枯渇時に空きを待つ上限。これを超えたら待ち続けず利用者へ混雑を返す。
_POOL_ACQUIRE_TIMEOUT_SECONDS = 5.0
# 枯渇待ちのポーリング間隔。短すぎると busy-loop、長すぎると体感待ちが伸びる。
...
class MySQLGreetingCrudRepository(GreetingCrudPort):
    ...
    def __init__(self, config: MySQLConfig) -> None:
        ...
        #
        # blocking=False とし、枯渇時の無制限待機（画面が固まる原因）を避ける。
        # PooledDB 自体には取得タイムアウトが無いため、空き待ちは _acquire_connection
        # 側で上限付き（_POOL_ACQUIRE_TIMEOUT_SECONDS）に制御する。
        self._pool = PooledDB(
            creator=pymysql,
    ...
    def _acquire_connection(self) -> pymysql.connections.Connection:
        """プールから接続を借りる。枯渇時は上限付きで待機し、超過なら混雑エラー。

        blocking=False のプールは枯渇時に即 TooManyConnectionsError を送出する。
        これをそのまま失敗にせず、_acquire_timeout_seconds の範囲で短い間隔の
        リトライにより空き接続を待つ。待機上限を超えたら、画面を無制限に固めず
        利用者へ「混雑」を伝えられるよう RepositoryError へ翻訳する。
        """
        ...
        while True:
            try:
                ...
                if time.monotonic() >= deadline:
                    logger.warning(
                        "接続プールが枯渇（待機上限 %.1fs 超過）",
                        self._acquire_timeout_seconds,
                    )
                    raise RepositoryError(
                        "ただ今アクセスが集中しています。少し待って再操作してください"
                    ) from error
    ...
    def _cursor(
        ...
        システム障害の RepositoryError とする。接続取得（枯渇・接続不可）の
        翻訳は _acquire_connection に委ねる。
        """
